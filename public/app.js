...
const weatherHref = `https://www.google.com/search?q=${encodeURIComponent('weather ' + c.name)}`;
if (!w || w.error) {
...
<p style="margin:8px 0 0 0; font-size:12px; color:var(--muted);">Weather lands at the next refresh (use ↻ Refresh).</p>
`,
...
const href = `https://finance.yahoo.com/quote/${encodeURIComponent(meta.symbol)}`;
const symBlock = `<a href="${esc(href)}" target="_blank" rel="noopener" class="row-link"><div class="sym">${esc(meta.symbol)}</div><div class="name">${esc(meta.name || '')}</div></a>`;
if (!data || data.error) {
...
title: 'Add a widget',
body: `
<p style="margin:0 0 10px 0; font-size:14px; color:#3f3f46;">Pick a type. Both save instantly to your account.</p>
...
const href = ev.htmlLink || 'https://calendar.google.com/';
return `<a href="${esc(href)}" target="_blank" rel="noopener" class="cal-item row-link"><div class="cal-time">${when}</div><div><div class="cal-title">${esc(ev.summary || '(no title)')}</div>${loc}</div></a>`;
}).join('');
} catch (e) {
...
const href = `https://mail.google.com/mail/u/0/#inbox/${esc(t.id)}`;
return `<a href="${esc(href)}" target="_blank" rel="noopener" class="email-item row-link">
